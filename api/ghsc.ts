...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const label = (req.query.label as string)?.trim() || 'Stars'

  if (!user || !repo) {
    res.status(400).send('Usage: /ghsc?user=<username>&repo=<repository>&label=Stars')
    return
  }
  ...
  const valueText = stars.toLocaleString()
...
<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="20">
  ...
  <g mask="url(#a)">
    ...
    <rect x="${labelWidth}" width="${valueWidth}" height="20" fill="#ffcc00"/>
