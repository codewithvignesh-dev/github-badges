...
module.exports = async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const label = (req.query.label as string)?.trim() || 'Forks'

  if (!user || !repo) {
    res.status(400).send('Usage: /ghforks?user=<username>&repo=<repository>&label=Forks')
    return
  }
  ...
  const valueText = forks.toLocaleString()
...
<svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="20">
  ...
  <g mask="url(#a)">
    ...
    <rect x="${labelWidth}" width="${valueWidth}" height="20" fill="#a2aaad"/>
